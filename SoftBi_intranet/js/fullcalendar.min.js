...
function DaysNumberofDate(DateGL) {
	var DateFormat = new Date(DateGL);
	return parseInt((Date.parse(DateFormat) - Date.parse(DateFormat.getFullYear() + "/1/1")) / 86400000) + 1;
}
...
function DaysNumberofMonth(DateGL) {
	var DateFormat = new Date(DateGL);
	var MM1 = DateFormat.getFullYear();
	MM1 < 100 ? MM1 += 1900 : MM1;
	var MM2 = MM1;
	MM1 += "/" + (DateFormat.getMonth() + 1);
	MM2 += "/" + (DateFormat.getMonth() + 2);
	MM1 += "/1";
	MM2 += "/1";
...
function SolarTerm(DateGL) {
	...
	var DateFormat = new Date(DateGL);
	for (;DateFormat.getFullYear() < BeginTime.getFullYear();) {
		BeginTime.setTime(BeginTime.getTime() - DifferenceInYear * 1000);
	}
	for (;DateFormat.getFullYear() > BeginTime.getFullYear();) {
		BeginTime.setTime(BeginTime.getTime() + DifferenceInYear * 1000);
	}
	for (var M = 0;DateFormat.getMonth() > BeginTime.getMonth();M++) {
		BeginTime.setTime(BeginTime.getTime() + DifferenceInMonth[M] * 1000);
	}
	if (DateFormat.getDate() > BeginTime.getDate()) {
		BeginTime.setTime(BeginTime.getTime() + DifferenceInMonth[M] * 1000);
		M++;
	}
	if (DateFormat.getDate() > BeginTime.getDate()) {
...
var DayGrid = FC.DayGrid = Grid.extend(DayTableMixin, {
	...
	renderNumberCellHtml: function(date) {
		...
		var cnMonth = CnMonthofDate(date);//農曆月份
		var cnDay = CnDayofDate(date);//農曆日字
		...
		/* switch (cnMonDay) {
			...
			case '五月初六': holiday = '端午';break;
			case '八月十五': holiday = '中秋';break;
			case '九月初九': holiday = '重陽';break;
			case '臘月三十': holiday = '除夕';break;
		} */
		html += '<span class="solarday">' + cnDay + '</span>';
		html += '<span class="holiday">' + holiday + '</span>';
		html += '</td>';
...
fcViews.list = {
	...
	defaults: {
		buttonText: '清單', // text to display for English
